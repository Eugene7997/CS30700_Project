...
const Search = (props) => {
  ...
  const Fetchdata = async () => {
    ...
    if (window.checked === true) {
      response = await fetch('http://44.209.88.168:8000/arg/avg_api/', {
        method: 'POST',
      ...
      response = await fetch('http://44.209.88.168:8000/arg/api/', {
        method: 'POST',
...
const Choropleth = (props) => {
  ...
  const ChoroplethMap = (props) => {
    ...
    const fetchGeoData = async () => {
      ...
      if (window.checked === true) {
        const response = await fetch('http://44.209.88.168:8000/arg/avg_geojson/', {
          method: 'POST',
